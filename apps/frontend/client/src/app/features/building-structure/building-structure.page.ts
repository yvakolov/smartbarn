            ...
            @for (storey of structure().storeys; track storey.id; let i = $index) {
              <article class="rounded-lg border border-[var(--sb-border)] bg-[var(--sb-surface)] p-5">
                ...
                <div class="mt-4 grid gap-3 sm:grid-cols-3">
                  ...
                  <label class="text-sm">{{ 'structure.storeyHeight' | transloco }}<input type="number" class="mt-1 w-full rounded border border-[var(--sb-border)] bg-transparent p-2" [value]="storey.heightMm" (change)="updateStoreyHeight(storey.id,$event)" /></label>
...
export class BuildingStructurePage {
  ...
  addStorey(): void {
    ...
    this.commit({ ...current, storeys: [...current.storeys, { id: `storey-${Date.now()}`, name: `Этаж ${index}`, baseElevationMm, heightMm: 3000 }] });
  }
  ...
  updateStoreyHeight(id: string, event: Event): void {
    const current = this.structure();
    const storeys = current.storeys.map((storey) => storey.id === id ? { ...storey, heightMm: Math.max(1800, this.readNumber(event, storey.heightMm)) } : storey);
